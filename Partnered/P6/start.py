...
def compile(path):
    try:
        result = subprocess.run(['make', 'all'], cwd=path, check=True,
...
def run(args):
    ...
    try:
        ...
        proxy_process = run_proxy(
            proxy_ports, num_listener, num_worker, server_host, server_port, q, test_home)
    except:
...
def main():
    ...
    parser.add_argument('--host', type=str, default='localhost',
                        help='Target host of the request')
    parser.add_argument('-l', '--lnum', type=int, default=1,
                        help='Number of listener threads')
    parser.add_argument('-w', '--wnum', type=int, default=1,
                        help='Number of worker threads')
    parser.add_argument('-p', '--port', type=int,
                        help='Target port of the request')
    parser.add_argument('-q', '--qsize', default=10,
                        type=int, help='Maximum size of the pq')
    ...
    parser.add_argument('--testhome', type=str, default='./',
                        help='Where the source files of the solution are located. There should also be a Makefile in this directory with at least the `all` target')
    parser.add_argument('--targethome', type=str, default='./public_html',
                        help='The target server runs in this directory. File requests are resolved relative to this directory.')
